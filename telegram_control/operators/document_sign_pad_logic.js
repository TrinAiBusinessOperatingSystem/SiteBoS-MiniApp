 ...
 * Compliance Route: EXTERNAL_PAD (Via A)
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const COMPLIANCE_ROUTE = 'EXTERNAL_PAD';
    ...
    const stepName = urlParams.get('step_name') || urlParams.get('step_title') || urlParams.get('context') || 'Firma Tavoletta Digitale';
        ...
        if (!templateId) {
            previewContent.innerHTML = `
                <div class="p-3 text-gray-700 bg-white rounded-lg border border-gray-200">
                    <p class="font-bold text-slate-900 mb-1">Modulo di Presa Visione e Sottoscrizione</p>
                    <p>Convalida esterna/pad per l'attività: <strong>${stepName}</strong> (Job ${jobId}).</p>
                </div>
            `;
            return;
        }
    ...
    async function requestCustomerSignature() {
        ...
        if (window.OfflineQueue && typeof window.OfflineQueue.executeOrEnqueue === 'function') {
            ...
            btnActionPrimary.innerHTML = `<i class="fa-solid fa-paper-plane text-amber-400"></i><span>Invia Richiesta di Firma al Cliente</span>`;
