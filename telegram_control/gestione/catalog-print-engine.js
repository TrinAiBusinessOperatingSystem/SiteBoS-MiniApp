...
const CatalogPrintEngine = {
    ...
    printSingleService: async function (payload) {
        ...
        if (adv.financial_advisory) {
            currentPageNum++;
            const fa = adv.financial_advisory || {};
            const cb = fa.cost_breakdown_unit || {};
            const sim = fa.predictive_volume_simulation || {};
            const scenarios = sim.scenarios || [];
            const breakEvenUnits = sim.break_even_units_annually;
            const pricing = fa.pricing_summary || {};
            const cur = pricing.currency || 'EUR';
            const fiscalImpact = fa.fiscal_and_previdential_impact || {};

            const fmtCurrency = (val) => (val !== null && val !== undefined) ? `${typeof val === 'number' ? val.toFixed(2) : val} ${cur}` : 'N/D';
            ...
            html += `
                <div class="page">
                    <div>
                        ...
                        <div class="section-subtitle">Scomposizione Costi Unitari (Cost Breakdown)</div>
                        <div class="info-box" style="margin-top: 5px;">
                            <div class="grid-2">
                                <div>
                                    <div class="info-row">
                                        <span class="info-label">Materiali Diretti (COGS)</span>
                                        <span class="info-value">${fmtCurrency(cb.direct_materials_cost)}</span>
                                    </div>
                                    <div class="info-row">
                                        <span class="info-label">Lavorazioni Esterne</span>
                                        <span class="info-value">${fmtCurrency(cb.external_processing_cost)}</span>
                                    </div>
                                    <div class="info-row">
                                        <span class="info-label">Operatore Primario</span>
                                        <span class="info-value">${fmtCurrency(cb.primary_operator_fee)}</span>
                                    </div>
                                    <div class="info-row">
                                        <span class="info-label">Operatore Secondario</span>
                                        <span class="info-value">${fmtCurrency(cb.secondary_operator_fee)}</span>
                                    </div>
                                    <div class="info-row">
                                        <span class="info-label">Accantonamento Garanzia</span>
                                        <span class="info-value">${fmtCurrency(cb.guarantee_provision)}</span>
                                    </div>
                                </div>
                                <div>
                                    <div class="info-row">
                                        <span class="info-label">Totale Costi Variabili Diretti</span>
                                        <span class="info-value" style="color: #ef4444;">${fmtCurrency(cb.total_direct_variable_costs_unit)}</span>
                                    </div>
                                    <div class="info-row">
                                        <span class="info-label">Margine di Contribuzione Unitario</span>
                                        <span class="info-value" style="color: #16a34a;">${fmtCurrency(cb.contribution_margin_unit)}</span>
                                    </div>
                                    <div class="info-row">
                                        <span class="info-label">Costo Fisso Postazione / Sedia</span>
                                        <span class="info-value">${fmtCurrency(cb.workstation_time_cost_fixed)}</span>
                                    </div>
                                    <div class="info-row">
                                        <span class="info-label">Reddito Operativo Unitario</span>
                                        <span class="info-value" style="color: #1d4ed8; font-weight: 800;">${fmtCurrency(cb.operating_income_unit)}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ${cb.cost_calculation_rationale ? `
                            <div class="rationale-box" style="margin-top: 4px;">
                                <strong>Razionale di Calcolo Costi:</strong> ${cb.cost_calculation_rationale}
                            </div>
                        ` : ''}

                        <div class="section-subtitle" style="margin-top: 14px;">Scenari di Volume Mensili</div>
                        <table>
                            <thead>
                                <tr>
                                    <th>Parametro Finanziario</th>
                                    ${scenarios.map(s => `<th style="text-align: center;">Scenario ${s.monthly_volume !== undefined ? s.monthly_volume : 'N/D'} Unità</th>`).join('')}
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    <td><strong>Ricavi Totali Proiettati</strong></td>
                                    ${scenarios.map(s => `<td style="text-align: center; font-weight: 700;">${s.total_revenues !== undefined && s.total_revenues !== null ? `${typeof s.total_revenues === 'number' ? s.total_revenues.toFixed(2) : s.total_revenues} ${cur}` : 'N/D'}</td>`).join('')}
                                </tr>
                                <tr style="background-color: #f0fdf4;">
                                    <td><strong>Margine di Contribuzione Totale</strong></td>
                                    ${scenarios.map(s => `<td style="text-align: center; color: #16a34a; font-weight: 700;">${s.total_contribution_margin !== undefined && s.total_contribution_margin !== null ? `${typeof s.total_contribution_margin === 'number' ? s.total_contribution_margin.toFixed(2) : s.total_contribution_margin} ${cur}` : 'N/D'}</td>`).join('')}
                                </tr>
                                <tr style="background-color: #eff6ff; border-top: 2px solid #3b82f6;">
                                    <td><strong>Utile Operativo Stimato (EBIT)</strong></td>
                                    ${scenarios.map(s => `<td style="text-align: center; color: #1d4ed8; font-weight: 900;">${s.total_projected_operating_income !== undefined && s.total_projected_operating_income !== null ? `${typeof s.total_projected_operating_income === 'number' ? s.total_projected_operating_income.toFixed(2) : s.total_projected_operating_income} ${cur}` : 'N/D'}</td>`).join('')}
                                </tr>
                            </tbody>
                        </table>
                        ...
                        <div class="grid-2" style="margin-top: 10px;">
                            <div>
                                <div class="section-subtitle" style="margin-top: 0;">Punto di Pareggio (Break-Even)</div>
                                <div class="info-box" style="margin-top: 5px;">
                                    <div class="info-row">
                                        <span class="info-label">Unità Minime Annue al Pareggio</span>
                                        <span class="info-value" style="color: #1d4ed8; font-weight: 800;">
                                            ${breakEvenUnits !== null && breakEvenUnits !== undefined ? `${breakEvenUnits} Unità` : 'N/D'}
                                        </span>
                                    </div>
                                </div>
                            </div>
                            <div>
                                <div class="section-subtitle" style="margin-top: 0;">Impatto Fiscale & Previdenziale</div>
                                <div class="info-box" style="margin-top: 5px;">
                                    <div class="info-row">
                                        <span class="info-label">Regime di Tassazione</span>
                                        <span class="info-value">${fiscalImpact.assumed_taxation_regime || 'N/D'}</span>
                                    </div>
                                    <div class="info-row">
                                        <span class="info-label">Carico Fiscale/Previdenziale Stimato</span>
                                        <span class="info-value" style="color: #ef4444;">${fiscalImpact.estimated_tax_and_previdential_burden_value !== null && fiscalImpact.estimated_tax_and_previdential_burden_value !== undefined ? `${typeof fiscalImpact.estimated_tax_and_previdential_burden_value === 'number' ? fiscalImpact.estimated_tax_and_previdential_burden_value.toFixed(2) : fiscalImpact.estimated_tax_and_previdential_burden_value} ${cur}` : 'N/D'}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ${fiscalImpact.fiscal_calculation_rationale ? `
                            <div class="rationale-box" style="margin-top: 4px;">
                                <strong>Razionale Fiscale:</strong> ${fiscalImpact.fiscal_calculation_rationale}
                            </div>
                        ` : ''}
                    </div>
                    ${makeFooter(currentPageNum)}
        ...
        // ─── PAGINA 6: PROFILO FISCALE E BENCHMARK TERRITORIALE ───
        if (adv.fiscal_snapshot || adv.territory_kpi) {
            currentPageNum++;
            const tkpi = adv.territory_kpi || {};
            const fsnap = adv.fiscal_snapshot || {};
            const tari = fsnap.tari || {};
            const cciaa = fsnap.cciaa_fee || {};
            const envObs = fsnap.environmental_obligations || {};
            const regTariffs = fsnap.regional_tariffs_permits || {};

            const fmtRate = (val) => (val !== null && val !== undefined && val !== '') ? (typeof val === 'number' ? `${val.toFixed(2)} EUR/h` : `${val} EUR/h`) : 'N/D';
            const fmtText = (val) => (val !== null && val !== undefined && val !== '') ? val : 'N/D';
            ...
            html += `
                <div class="page">
                    <div>
                        <div class="header-doc">
                            <span>SKU: ${sku}</span>
                            <h2>PROFILO FISCALE & BENCHMARK TERRITORIALE</h2>
                        </div>
                        <div class="chapter-title">Quadro Fiscale, Normativo e Parametri Territoriali</div>

                        <div class="section-subtitle">Benchmark Territoriale e Parametri Orari</div>
                        ${tkpi.summary ? `
                            <p style="font-size: 8.5pt; color: #334155; line-height: 1.5; margin-bottom: 8px;">${tkpi.summary}</p>
                        ` : ''}
                        <div class="info-box" style="margin-top: 4px; padding: 8px 12px;">
                            <div class="info-row">
                                <span class="info-label">Tariffa Oraria Operatore Primario</span>
                                <span class="info-value">${fmtRate(tkpi.primary_operator_hourly_rate)}</span>
                            </div>
                            <div class="info-row">
                                <span class="info-label">Tariffa Oraria Operatore Secondario</span>
                                <span class="info-value">${fmtRate(tkpi.secondary_operator_hourly_rate)}</span>
                            </div>
                            <div class="info-row">
                                <span class="info-label">Costo Orario Postazione / Sedia</span>
                                <span class="info-value">${fmtRate(tkpi.workstation_hourly_rate)}</span>
                            </div>
                        </div>

                        <div class="section-subtitle" style="margin-top: 14px;">Profilo Fiscale & Tributi Locali</div>
                        <div class="info-box" style="margin-top: 4px; padding: 8px 12px;">
                            <div class="grid-2">
                                <div>
                                    <div class="info-row">
                                        <span class="info-label">Aliquota IRES</span>
                                        <span class="info-value">${fmtText(fsnap.ires_rate)}</span>
                                    </div>
                                    <div class="info-row">
                                        <span class="info-label">Aliquota IRAP</span>
                                        <span class="info-value">${fmtText(fsnap.irap_rate)}</span>
                                    </div>
                                    <div class="info-row">
                                        <span class="info-label">Diritto Annuale CCIAA</span>
                                        <span class="info-value">${fmtText(cciaa.amount)}${cciaa.due_date ? ` (Scadenza: ${cciaa.due_date})` : ''}</span>
                                    </div>
                                </div>
                                <div>
                                    <div class="info-row">
                                        <span class="info-label">TARI Quota Fissa</span>
                                        <span class="info-value">${fmtText(tari.quota_fissa)}</span>
                                    </div>
                                    <div class="info-row">
                                        <span class="info-label">TARI Quota Variabile</span>
                                        <span class="info-value">${fmtText(tari.quota_variabile)}</span>
                                    </div>
                                    <div class="info-row">
                                        <span class="info-label">Addizionale TEFA</span>
                                        <span class="info-value">${fmtText(tari.tefa_rate)}</span>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div class="section-subtitle" style="margin-top: 14px;">Obblighi Ambientali & Conformità</div>
                        <div class="info-box" style="margin-top: 4px; padding: 8px 12px;">
                            <div class="info-row">
                                <span class="info-label">Stato Iscrizione RENTRI</span>
                                <span class="info-value">${fmtText(envObs.rentri_status)}</span>
                            </div>
                            ${envObs.rentri_annual_fee ? `
                                <div class="info-row">
                                    <span class="info-label">Contributo Annuale RENTRI</span>
                                    <span class="info-value">${fmtText(envObs.rentri_annual_fee)}</span>
                                </div>
                            ` : ''}
                            ${envObs.special_waste_codes && envObs.special_waste_codes.length > 0 ? `
                                <div class="info-row">
                                    <span class="info-label">Codici CER Rifiuti Speciali</span>
                                    <span class="info-value">${envObs.special_waste_codes.join(', ')}</span>
                                </div>
                            ` : ''}
                            ${regTariffs.concessione_governativa ? `
                                <div class="info-row">
                                    <span class="info-label">Tassa Concessione Governativa</span>
                                    <span class="info-value">${fmtText(regTariffs.concessione_governativa)}</span>
                                </div>
                            ` : ''}
                            ${regTariffs.asp_inspection_fee ? `
                                <div class="info-row">
                                    <span class="info-label">Tariffa Ispezione ASP / ATS</span>
                                    <span class="info-value">${fmtText(regTariffs.asp_inspection_fee)}</span>
                                </div>
                            ` : ''}
                        </div>
                        ${envObs.environmental_rules ? `
                            <div class="rationale-box" style="margin-top: 5px;">
                                <strong>Normativa Ambientale di Riferimento:</strong> ${envObs.environmental_rules}
                            </div>
                        ` : ''}
                    </div>
                    ${makeFooter(currentPageNum)}
        ...
        // ─── PAGINA 7: CONSULENZA STRATEGICA E RACCOMANDAZIONI OPERATIVE ───
        const fa = adv.financial_advisory || {};
        if (fa && (fa.operations_financial_health_rating || fa.pricing_and_tariff_strategy_advisory || (fa.operations_cost_optimization_recommendations && fa.operations_cost_optimization_recommendations.length > 0))) {
            currentPageNum++;
            const hasRecommendations = fa.operations_cost_optimization_recommendations && fa.operations_cost_optimization_recommendations.length > 0;
            const ratingText = (fa.operations_financial_health_rating || "").replace(/_/g, ' ');
            ...
            html += `
                <div class="page">
                    <div>
                        ...
                        ${fa.operations_financial_health_rating ? `
                        ...
                        ${fa.pricing_and_tariff_strategy_advisory ? `
                        ...
                        ${hasRecommendations ? `
                            ...
                            <div style="display: flex; flex-direction: column; gap: 8px; margin-top: 6px;">
                                ${fa.operations_cost_optimization_recommendations.map((rec, rIdx) => `
